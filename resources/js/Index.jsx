...
export default function Index(props) {
    ...
    return (
        <AuthenticatedLayout
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Habits Insights</h2>}
        >
            <Head title="Habits" />
                    ...
                    return (
                        <>
                            <Card className="flex justify-between">
                                <div>
                                    <h3 className={"text-xl font-semibold "+color}><Link href={route('habits.show', habit.id)}>{habit.name}</Link></h3>
                            ...
                            {openedChildIndex === index && habit.children && habit.children.map((child, index) => {
                                ...
                                return (
                                    <Card className={"ml-5"}>
                                        <h3 className={"text-xl font-semibold "+color}><Link href={route('habits.show', child.id)}>{child.name}</Link></h3>
                                        <div className="text-4xl font-bold text-white">
